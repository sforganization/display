...
void ws2812bSet(const int index, const int32_t color)
{
    ...
    else
    {
        for(i = 0; i < WS2812B_NUM; i++){
            ...
            //WS2812B_CREATE_PULSE_BUFF();  //不用这个宏，当index为-1时会k = 0xffffffe8 ,从这里开始，是不是会清空了多余的几个字节的数据
            clr = color; 
            for (j = 0; j < WS2812B_PULSE_BIT_SIZE; j++) { 
                ws2812bBuff[i * WS2812B_PULSE_BIT_SIZE + j] = clr & 0x800000 ? WS2812B_PWM_HIGH_PULSE_WIDTH : WS2812B_PWM_LOW_PULSE_WIDTH; 
                clr <<= 1; 
            }
        }
    }
